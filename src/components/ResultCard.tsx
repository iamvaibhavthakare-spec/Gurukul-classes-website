  ...
  return (
    ...
    >
      ...
      <div className="mx-auto flex h-24 w-24 items-center justify-center overflow-hidden rounded-full bg-[#EAF1FF] text-[#2563EB] ring-8 ring-[#F8FAFF] transition-transform group-hover:scale-105 md:h-28 md:w-28">
        {result.photo ? (
          <img src={result.photo} alt={result.name} className="h-full w-full object-cover" loading="lazy" />
        ) : (
      ...
      <p className="mt-4 text-3xl font-extrabold leading-tight text-brand-ink md:text-[2.15rem]">{result.score}</p>
      <p className="mt-2 text-sm text-muted-foreground">{result.college}</p>
    </div>
  );
}
