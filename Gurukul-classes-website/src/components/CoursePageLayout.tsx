...
const LIVE_COURSE_DETAILS: Record<
  string,
  { duration: string; schedule: string; included: string[] }
> = {
  "viii-ssc-cbse": {
    ...
    included: [
      "Weekly tests",
      "Permanent progress reports",
      "Extra exam-time classes",
      "Personal attention for students who need support",
    ],
  },
  "ix-ssc-cbse": {
    duration: "Academic year foundation program",
    ...
    included: [
      ...
      "CBSE and SSC board-aligned preparation",
    ],
  },
  "x-ssc-cbse": {
  ...
  "mh-cet": {
    ...
    schedule:
      "Entrance topics covered with XI and XII syllabus, plus chapter and weekly cumulative tests",
    included: [
      "70% Entrance syllabus before XII boards",
      "20+ final-pattern mock tests",
      "All-India test series",
      "DPP and Entrance study material",
    ],
  },
  neet: {
  ...
  "iit-jee": {
    ...
    included: [
      "XI and XII integrated Entrance coverage",
...
export function CoursePageLayout({ course }: { course: Course }) {
  ...
  const courseDetails = [
    {
      title: "Duration",
      value: course.duration ?? liveDetails?.duration ?? "Full academic year with revision support",
    },
    {
      title: "Schedule",
      value:
        course.schedule ??
        liveDetails?.schedule ??
        "Weekly classroom lectures, tests and doubt-solving sessions",
    },
    {
      title: "What's Included",
      value: (course.included ?? liveDetails?.included ?? course.highlights.slice(0, 3)).join(", "),
    },
  ];
  return (
    <>
        ...
        <div className="container mx-auto px-4 grid lg:grid-cols-[1.4fr_1fr] gap-12">
          <div>
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              {courseDetails.map((item) => (
                <div
                  key={item.title}
                  className="rounded-2xl border border-[#2563EB]/15 bg-[#F8FAFF] p-5 shadow-soft"
                >
                  <p className="text-xs font-bold uppercase tracking-widest text-[#2563EB]">
                    {item.title}
                  </p>
                  <p className="mt-2 text-sm font-semibold leading-relaxed text-brand-ink/85">
                    {item.value}
                  </p>
                </div>
              ))}
            ...
            <div className="mt-10 rounded-2xl border border-[#2563EB]/20 bg-[#F8FAFF] p-6 md:p-8">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {course.subjects.map((s) => (
                  <span
                    key={s}
                    className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-semibold text-brand-ink shadow-soft"
                  >
          ...
          <div id="enquire" className="lg:sticky lg:top-28 self-start">
            <EnquiryForm
              ...
              subtitle={`Get details on fees, batches & syllabus for ${course.title}.`}
            />
          </div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-24 bg-[#F6F8FA]">
        <div className="container mx-auto px-4">
          <SectionTitle
            ...
            description="Find the right course for your next academic milestone."
          />
