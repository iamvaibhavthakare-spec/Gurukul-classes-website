...
export function EnquiryForm({ variant = "enquiry", className, title, subtitle }: Props) {
  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const subject = encodeURIComponent(
      `${variant.toUpperCase()} — ${obj.name || obj.studentName || "Website"}`,
    );

    // Open WhatsApp in a new tab as the primary channel
    window.open(`https://wa.me/${SITE.whatsapp}?text=${body}`, "_blank", "noopener,noreferrer");
    // Mailto fallback
  ...
  return (
    <form
      onSubmit={onSubmit}
      className={cn("rounded-2xl border border-border bg-white p-6 md:p-8 shadow-card", className)}
    >
      {(title || subtitle) && (
        <div className="mb-6">
