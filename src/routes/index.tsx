import { ArrowRight, Award, BookOpenCheck, CalendarCheck, ChevronRight, ClipboardCheck, GraduationCap, MonitorPlay, PlayCircle, Quote, Sparkles, Target, Trophy, Users } from "lucide-react";
      ...
      <section className="py-14 bg-white">
        <div className="container mx-auto px-4">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h2 className="mt-4 text-3xl font-extrabold text-brand-ink md:text-5xl">Start with the right preparation path</h2>
            <p className="mt-3 text-muted-foreground">Choose the course track that matches your class, exam and college ambition.</p>
          </div>
      ...
      <section className="py-16 md:py-20 bg-[#F6F8FA]">
        <div className="container mx-auto px-4">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            <div>
              <span className="text-xs font-bold uppercase tracking-widest text-[#2563EB]">Why students stay consistent</span>
              <h2 className="mt-4 text-3xl font-extrabold text-brand-ink md:text-5xl">Everything needed for steady exam preparation</h2>
              <p className="mt-4 text-muted-foreground leading-relaxed">
                Inspired by modern learning platforms, Gurukul keeps the experience simple:
                clear goals, regular practice and mentors who stay involved.
              </p>
            </div>
            <div className="grid gap-4">
              {LEARNING_FEATURES.map(({ Icon, title, text }) => (
                <div key={title} className="flex gap-4 rounded-2xl border border-border bg-white p-5 shadow-soft">
          ...
          <div className="rounded-2xl border border-[#2563EB]/20 bg-[#F8FAFF] p-0 shadow-soft">
            <div className="rounded-[1.4rem] bg-white p-6 md:p-8 grid md:grid-cols-[1.4fr_1fr] gap-6 items-center">
              ...
              <div className="flex flex-wrap gap-3 md:justify-end">
                <a href="#enquire" className="inline-flex items-center gap-2 rounded-lg bg-[#2563EB] px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-[#1D4ED8]">
                  Enquire Now <ArrowRight className="h-4 w-4" />
                </a>
                <a href={`tel:${SITE.primaryPhoneRaw}`} className="inline-flex items-center gap-2 rounded-lg border border-brand-ink/10 px-6 py-3 text-sm font-bold text-brand-ink hover:border-[#2563EB] hover:text-[#2563EB]">
                  {SITE.primaryPhone}
                </a>
              </div>
            </div>
        ...
        <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-12 items-center">
          <div className="relative">
            ...
            <div className="relative grid grid-cols-2 gap-4">
              <img src={aboutTeaching} alt="Teacher with students" className="rounded-2xl shadow-card object-cover h-80 w-full" loading="lazy" />
              <div className="space-y-4">
                <img src={heroClassroom} alt="Modern classroom" className="rounded-2xl shadow-card object-cover h-36 w-full" loading="lazy" />
          ...
          <div>
            ...
            <Link to="/about" className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-[#2563EB] hover:gap-3 transition-all">
      ...
      <section className="py-16 md:py-24 bg-[#F6F8FA]">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {WHY_CHOOSE.map((w, i) => (
              <div key={w.title} className="rounded-2xl border border-border bg-white p-6 shadow-soft transition-transform hover:-translate-y-1 hover:shadow-card">
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="container mx-auto px-4">
          ...
          <div className="mt-10 text-center">
            <Link to="/results" className="inline-flex items-center gap-2 rounded-lg bg-[#2563EB] px-6 py-3 text-sm font-bold text-white shadow-card hover:bg-[#1D4ED8]">
      ...
      <section className="py-16 md:py-24 bg-[#111827] text-white relative overflow-hidden">
        <div className="container relative mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {METHODOLOGY.map((m) => (
              <div key={m.step} className="relative rounded-2xl border border-white/10 bg-white/[0.06] p-6 transition-colors hover:bg-white/10">
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <Link to="/videos" className="mt-6 inline-flex items-center gap-2 rounded-lg bg-[#2563EB] px-6 py-3 text-sm font-bold text-white shadow-card hover:bg-[#1D4ED8]">
            ...
            <div className="mt-8 grid sm:grid-cols-2 gap-4">
              {BRANCHES.slice(0, 4).map((b) => (
                <div key={b.slug} className="rounded-2xl border border-border bg-white p-4 shadow-soft">
                  <p className="text-xs font-bold tracking-widest text-[#2563EB] uppercase">{b.area}</p>
                  <p className="mt-1 font-bold text-brand-ink">{b.name}</p>
                  <a href={`tel:${b.phoneRaw}`} className="mt-2 text-sm text-muted-foreground hover:text-[#2563EB]">{b.phone}</a>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="relative overflow-hidden rounded-2xl bg-[#111827] p-10 md:p-14 text-white shadow-card">
            <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              ...
              <a href="#enquire" className="inline-flex items-center gap-2 rounded-lg bg-white px-7 py-4 text-sm font-extrabold text-brand-ink shadow-soft hover:bg-[#EAF1FF]">
    ...
    <section className="py-16 md:py-24 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="mt-8 text-center">
          <Link to="/gallery" className="inline-flex items-center gap-2 text-sm font-bold text-[#2563EB] hover:gap-3 transition-all">
